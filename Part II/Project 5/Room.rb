...
def create_room
  "You are in a #{size} #{color} #{room_atmosphere} #{room_type}. There is an exit on the #{direction} wall."
  end
  def size
    ["huge", "large", "big", "regular", "small", "tiny", "deep"].sample
end
def color
  ["red", "blue", "green", "dark", "golden", "crystal", "grey"].sample
end
def room_type
  ["cave", "treasure room", "rock cavern", "tomb", "guard room", "lair","prison room"].sample
end
def room_atmosphere
  ["quite","noisey","daunting","flooded","muddy","old"].sample
end
def direction
